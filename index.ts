...
async function handleNotification(body: any) {
  ...
  if (fangtangKey) {
    const wxTitle = "DukeParents 有 1 条待审批申请";
    const wxDesp = `待处理申请：1 条\n最新：${type === "invite_request" ? (classVal + "届 " + (wxgroup || submittedBy)) : title}\n\n[👉 点击进入审批页面](https://dukeparents.org/approve.html)`;
    await fetch(`https://sctapi.ftqq.com/${fangtangKey}.send`, {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: new URLSearchParams({ title: wxTitle, desp: wxDesp }),
    }).catch(() => {});
  }
